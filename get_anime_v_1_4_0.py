...
class AnimePlayerApp:
    ...
    def read_json_data(self):
        try:
            ...
            self.log_message("File 'response.json' not found.")
            return None
        except json.JSONDecodeError as e:
            error_message = f"Error decoding JSON data from 'response.json': {str(e)}"
    ...
    def save_playlist(self):
        ...
        if data is not None:
            if "list" in data and isinstance(data["list"], list) and len(data["list"]) > 0:
                ...
                self.playlist_name = os.path.join(current_directory, f"{data['list'][0]['code']}.m3u")
    ...
    def all_links_play(self):
        try:
            ...
            if hasattr(self, 'playlist_name') and self.playlist_name is not None:
                media_player_command = [vlc_path, self.playlist_name]
                subprocess.Popen(media_player_command)
            else:
    ...
    def delete_response_json(self):
        try:
            ...
            self.log_message(f"An error occurred while deleting 'response.json': {str(e)}")
