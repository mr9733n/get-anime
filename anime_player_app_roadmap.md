...
- [ ] Троттлинг скорости (лимит КБ/c) — опция в GUI.
- [ ] Перезапуск приёмника при сбое mDNS/сокета (авто-рестарт).
- [ ] Резюмируемая отправка (resume from offset): сверка размера/хеша снапшота и докачка недостающих чанков.
- [ ] Доп. контроль целостности на каждый чанк (CRC32/sha256) + перезапрос конкретных чанков.
...
- [ ] Кнопки «Сохранить лог» и «Очистить лог».
- [ ] Кнопка «Сбросить доверие (TOFU)» для выбранного хоста.
- [ ] Настройки: лимит скорости, размер чанка, опция сжатия, флаги merge (см. ниже) — сохранять в конфиг.
...
- [+] Итоговый отчёт: число пропущенных по каждой таблице + экспорт CSV «сироты/дубликаты».
- [ ] Чекбоксы в GUI Merge: 
...
- [ ] `PRAGMA optimize`/`VACUUM` после большого мержа (по чекбоксу).
