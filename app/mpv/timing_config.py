...
class TimingConfig:
    ...
    if DEBUG_MODE:
        ...
        FORCE_RELOAD_WATCHDOG_PAUSE = 2000 # Пауза watchdog после force-reload
        ...
        # ↑ было 300: с cache-pause-initial MPV сам держит паузу пока не накоплен
        #   буфер, поэтому жёстко увеличивать не нужно — достаточно запаса на парсинг.
        METADATA_LOAD_WAIT = 600  # ← было 300: запас перед вызовом play()
        PLAYBACK_START_WAIT = 300  # ← Больше времени на стабилизацию

        # ↓ было 2000: watchdog должен терпеливее ждать после seek/reload —
        #   CDN может отвечать несколько секунд, cache-pause отработает сам.
        WATCHDOG_PAUSE = 4000              # Пауза watchdog после seek
        # После force-reload нужно больше времени: stream заново подключается,
        # MPV парсит манифест, cache-pause-initial накапливает буфер.
        FORCE_RELOAD_WATCHDOG_PAUSE = 6000 # Пауза watchdog после force-reload
        SEEKING_GUARD = 3000               # Защита от преждевременных seek

        ZERO_DELAY = 0
        SWITCHING_TRACK = 400
        NEXT_MEDIA = 200
        FORCE_RELOAD_CUR = 1500  # было 900: больше времени до seek после reload
        VIDEO_WIN_USER_CLOSE = 10
    ...
    def info(cls):
        ...
        print(f"Watchdog Pause (seek):       {cls.WATCHDOG_PAUSE}ms")
        print(f"Watchdog Pause (reload):     {cls.FORCE_RELOAD_WATCHDOG_PAUSE}ms")
        print(f"Force Reload Cur:            {cls.FORCE_RELOAD_CUR}ms")
        print("=" * 50)
...
FORCE_RELOAD_WATCHDOG_PAUSE = TimingConfig.FORCE_RELOAD_WATCHDOG_PAUSE
