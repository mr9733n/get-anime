...
class FranchiseRelease(Base):
    ...
    title_id = Column(Integer, ForeignKey('titles.title_id'), nullable=False)
    ext_fr_id = Column(String, nullable=True)
    ext_fr_rel_id = Column(String, nullable=True)
    ext_rel_id = Column(Integer, nullable=True)
...
class Torrent(Base):
    ...
    range_first = Column(Integer)
    range_last = Column(Integer)
    ...
    api_updated_at = Column(DateTime, default=datetime.now(timezone.utc))
    is_in_production = Column(Integer)
